        ...
        function toWGS(networks, filename)
            ...
            for k = 1:n_net
                ...
                if ~isfield(net, 'name') || isempty(net.name)
                    net.name = sprintf('Network_%02d', k);
                end
                if ~isfield(net, 'iSymX') || isempty(net.iSymX)
                    net.iSymX = 0;
                end
                if ~isfield(net, 'iSymZ') || isempty(net.iSymZ)
                    net.iSymZ = 0;
                end

                if any(~isfinite(net.X(:))) || any(~isfinite(net.Y(:))) || any(~isfinite(net.Z(:)))
                    error(['MeshWriter.toWGS: network "%s" contains non-finite values. ' ...
                           'WGS export requires a full structured network.'], net.name);
                end
